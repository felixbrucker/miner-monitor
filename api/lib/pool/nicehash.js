...
module.exports = {
  poolStats: async (address, exchangeRates) => {
  ...
  balance: async (userId, apiKey) => {
    const balanceData = await axios.get(`https://api.nicehash.com/api?method=balance&id=${userId}&key=${apiKey}`);
