function WalletController() {
  ...
  ctrl.isSynced = (data) => {
    return moment().diff(moment.unix(data.lastBlockReceived), 'minutes') < 30 && (data.syncProgress ? data.syncProgress >= 0.9997 : true);
  };

  ctrl.isSyncing = (data) => {
    return (data.syncProgress ? data.syncProgress < 0.9997 : false);
  };
